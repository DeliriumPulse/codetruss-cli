...
2. Create and push the matching annotated tag, for example `v0.2.22` for package
   version `0.2.22`.
...
4. Verify the downloaded archive independently:
   ...
   ```bash
   gh attestation verify codetruss-cli-0.2.22.tgz --repo DeliriumPulse/codetruss-cli
   shasum -a 256 -c codetruss-cli-0.2.22.tgz.sha256
   ```
